...
const ROUNDS = [
  ...
  { id: "QF", label: "Quarterfinals", matchups: 4, multiplier: 3 },
  { id: "SF", label: "Semifinals", matchups: 2, multiplier: 4 },
  { id: "F",  label: "Final", matchups: 1, multiplier: 5 }
];
...
function setHeaderEnabled(enabled) {
  ...
  roundBtn.disabled = !enabled;
  counterPill.style.display = enabled ? "inline-flex" : "none";
  roundBtn.style.display = enabled ? "inline-flex" : "none";
...
function ensureNextRoundIfReady(roundIdJustCompleted) {
  ...
  if (!next) {
    // Champion decided
    active.bracket.currentRoundId = "F";
    return;
  }
...
function renderBracketPage() {
  ...
  roundBtn.textContent = `Round ${roundId}`;
  roundBtn.disabled = false;

  const tabsHtml = `
    <div class="roundTabs" role="tablist" aria-label="Rounds">
      ${ROUNDS.map(r => {
        const enabled = isRoundUnlocked(r.id);
        const isActive = r.id === roundId;
        return `<button class="roundTab ${isActive ? "isActive" : ""}" type="button"
                  data-round="${r.id}" aria-disabled="${enabled ? "false" : "true"}">${escapeHtml(r.id)}</button>`;
      }).join("")}
    </div>
  `;
  ...
  // wire tabs
  appEl.querySelectorAll("[data-round]").forEach(btn => {
...
function renderMatchCard(roundId, m, idx) {
  ...
  const decidedLabel = decided && m.decidedAt ? `Completed ${formatTime12(new Date(m.decidedAt))}` : "Not decided";
  ...
  return `
    <div class="matchCard">
      <div class="matchHeader">
        <div class="matchTitle">${escapeHtml(roundMeta.label)} · Match ${idx + 1}</div>
        <div class="matchMeta">${escapeHtml(decidedLabel)}</div>
      </div>

      <div class="pickRow">
        <button class="pickBtn ${aWinner ? "isWinner" : ""} ${aLoser ? "isLoser" : ""}"
          type="button" data-round="${roundId}" data-match="${m.id}" data-pick="${m.a}" ${decided ? "" : ""}>
      ...
      <div class="afterRow">
        <div>
          ...
          <div class="smallText">${escapeHtml(decided ? "Advances" : "Pick a winner to advance")}</div>
        </div>
...
function handlePickWinner(roundId, matchId, pickId) {
  ...
  // Tweet
  const tweet = buildDecisionTweet(roundId, winner, loser, pts);
...
function buildDecisionTweet(roundId, winnerId, loserId, points) {
  const w = shortNameFor(winnerId);
  const l = shortNameFor(loserId);
  const decided = countDecisions(active);

  // simple tweet text (you'll refine copy later)
  return `${roundId}: ${w} defeats ${l} (+${points}) • ${decided}/31 decided`;
}
